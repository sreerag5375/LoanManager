...
const LoanDashboard = () => {
        ...
        return (
            <div className="relative flex items-center justify-center">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none p-6 text-center animate-in fade-in duration-500">
                    {selectedSegment ? (
                        <>
                            <p className="text-xs font-semibold text-slate-400 uppercase mb-1 tracking-widest">{selectedSegment.name}</p>
                            <h3 className="text-3xl font-semibold text-slate-800" style={{ color: selectedSegment.color }}>₹{(selectedSegment.amount / 1000).toFixed(1)}k</h3>
                            <p className="text-xs font-medium text-slate-400 mt-1">{((selectedSegment.amount / totalAmount) * 100).toFixed(1)}% of total</p>
                        </>
                    ) : (
                        <>
                            <p className="text-xs font-semibold text-slate-400 uppercase mb-1 tracking-widest">Total Due</p>
                            <h3 className="text-4xl font-semibold text-slate-800">₹{(totalAmount / 1000).toFixed(0)}k</h3>
    ...
    const LoanCircle = ({ loan, index, isFirst }) => {
        ...
        return (
            ...
            >
                {/* Rank Badge */}
                <div className={`mb-2 sm:mb-4 px-2 sm:px-4 py-1 rounded-full text-[10px] sm:text-xs font-semibold uppercase tracking-wider ${isFirst ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-600'}`}>
                    #{index + 1}
                </div>
                ...
                <div className="relative w-full max-w-[90px] sm:max-w-[180px] aspect-square">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-2 sm:p-4">
                        ...
                        <h3 className="text-xs sm:text-2xl font-semibold text-slate-900 leading-tight">₹{(loan.amount / 1000).toFixed(1)}k</h3>
                        <p className="text-[8px] sm:text-xs font-semibold text-slate-400 mt-0.5 sm:mt-1">
    ...
    return (
        <div className="min-h-screen bg-white font-sans text-slate-800 pb-24" onClick={() => setSelectedSegment(null)}>
            ...
            <div className="bg-blue-50 pb-12 rounded-b-[3rem] shadow-sm relative overflow-hidden">
                ...
                <nav className="px-6 py-6 flex items-center justify-between max-w-5xl mx-auto relative z-10">
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="font-semibold text-2xl text-slate-900 tracking-tight">DebtDash</h1>
                    </div>
                </nav>
                ...
                <div className="max-w-5xl mx-auto px-6 mt-4">
                    <div className="flex flex-col-reverse md:flex-row items-center justify-center md:justify-between gap-12">
                        <div className="text-center md:text-left flex-1 relative z-10">
                            ...
                            <h2 className="text-6xl font-semibold text-slate-900 tracking-tight mb-6">₹{totalAmount.toLocaleString('en-IN')}</h2>
                            <div className="inline-flex items-center gap-2 bg-white/60 backdrop-blur-md border border-white/50 px-4 py-2.5 rounded-full shadow-sm">
                                ...
                                <span className="text-sm font-semibold text-slate-700">{loans.length} Active Accounts</span>
                            </div>
                        </div>
                    ...
                    <div className="flex flex-col gap-4">
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="text-right">
                                        <p className="font-semibold text-slate-900 text-xl">₹{loan.amount.toLocaleString()}</p>
                                    </div>
                                </div>
                            );
                        })}
                        <div className="bg-slate-50 p-5 rounded-2xl border-2 border-dashed border-slate-200 flex items-center justify-between mt-2">
                            <div className="flex items-center gap-4">
                                ...
                                <span className="font-semibold text-slate-500 text-lg uppercase tracking-wider">Total Outstanding</span>
                            </div>
                            <p className="font-semibold text-slate-900 text-2xl">₹{totalAmount.toLocaleString()}</p>
                        </div>
                    </div>
                </section>
            </div>
            ...
            {/* --- ADD BUTTON --- */}
            <button onClick={(e) => { e.stopPropagation(); setIsEditMode(false); setIsModalOpen(true); }} className="fixed bottom-24 right-8 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl p-4 shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all z-50 group">
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-slate-900/20 backdrop-blur-sm flex items-center justify-center p-4 z-[60]" onClick={closeModal}>
                    <div className="bg-white rounded-3xl shadow-2xl max-w-sm w-full p-8 animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                        <h3 className="text-xl font-semibold text-slate-900 mb-6">{isEditMode ? 'Edit Loan' : 'Add New Loan'}</h3>
                        ...
                        <input type="number" className="w-full bg-slate-50 border-none rounded-xl px-4 py-3 mb-6 font-semibold text-xl text-slate-900 placeholder-slate-400 focus:ring-2 focus:ring-blue-500" placeholder="₹ Amount" value={loanAmount} onChange={e => setLoanAmount(e.target.value)} />
                        <div className="flex gap-3">
                            {isEditMode && (<button onClick={() => { setDeletingLoan(editingLoan); setShowDeleteConfirm(true); setIsModalOpen(false); }} className="px-4 py-3 bg-red-50 text-red-500 rounded-xl font-semibold hover:bg-red-100 transition-colors"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>)}
                            <button onClick={saveLoan} className="flex-1 bg-blue-600 text-white rounded-xl font-semibold py-3 hover:bg-blue-700 transition-colors shadow-lg shadow-blue-200">Save Changes</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {showDeleteConfirm && (
                <div className="fixed inset-0 bg-slate-900/20 backdrop-blur-sm flex items-center justify-center p-4 z-[70]" onClick={() => setShowDeleteConfirm(false)}>
                    <div className="bg-white rounded-3xl p-8 max-w-xs w-full text-center shadow-2xl" onClick={e => e.stopPropagation()}>
                        <p className="font-semibold text-lg mb-2">Delete this loan?</p>
                        <p className="text-slate-500 mb-6">This action cannot be undone.</p>
                        <div className="flex gap-3">
                            <button onClick={() => setShowDeleteConfirm(false)} className="flex-1 py-2 rounded-xl font-semibold text-slate-500 hover:bg-slate-50">Cancel</button>
                            <button onClick={() => deleteLoan(deletingLoan?._id)} className="flex-1 py-2 rounded-xl font-semibold bg-red-500 text-white hover:bg-red-600 shadow-lg shadow-red-200">Delete</button>
                        </div>
                    </div>
                </div>
            )}
